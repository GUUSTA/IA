...
from Aux import imprime_atual, imprime_sucessores
import heapq
...
def a_estrela(estado_inicial, testar_objetivo, gerar_sucessores, heuristica, custo, imprimir=str, stepEstado=False, stepSucessores=False):
    ...
    while not fila.esta_vazio():
        ...
        if stepEstado:
            imprime_atual(estado_atual, imprimir)
        ...
        for estados_vertices_sucessor in estados_vertices_sucessores:
            ...
            if estado_filho not in visitados or visitados[estado_filho] > novo_custo:
                visitados[estado_filho] = novo_custo
                fila.push(No(estado_filho, no_atual, vertice,
                             novo_custo, heuristica(estado_filho)))
    return None
